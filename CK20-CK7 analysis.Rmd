...
ggplot(data=numInteractsByImage, aes(x=reorder(image, -num_interactions), y=num_interactions)) +
...
interAreaByImage <- merge(interAreaByImage, select(clinData, pad_ck, tum_percent, id), by.x = "image", by.y = "pad_ck") %>% relocate(tum_percent, .before = num_interactions)
...
ggplot(data=densityInterByImage, aes(x=reorder(image, -density_interacts), y=density_interacts)) +
  geom_bar(stat="identity", fill="steelblue")+
  theme_minimal() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggplot(data = numInterbyImageDist, aes(x = dist_to_tumborder_cat, y = num_interactions, group = image, colour = image)) + 
